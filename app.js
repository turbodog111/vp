...
/** Map library filename → spatial JSON slug (single primary track). */
...
function normalizeSpatialMap(data) {
  const map = data && data.keyframes ? data : { keyframes: data?.keyframes || SPATIAL_MAP_EMBEDDED.keyframes, transitionSec: data?.transition_sec || data?.transitionSec || 1.15 };
...
// Primary (only) spatial track map — jumpy moving leads
